...
async function supabaseQuery(table: string, method: string, body?: any, query?: string) {
  ...
  try {
    ...
    if (method === "PUT") return (await axios.put(url, body, { headers })).data;
...
// ✅ NEW: Message ID စစ်ပြီး duplicate block လုပ်မယ်
...
async function getOrCreateCustomer(psid: string) {
  ...
  const newCustomer = await supabaseQuery("customers", "POST", { psid });
...
async function getConversationHistory(customerId: number, limit: number = 10) {
  ...
  return (await supabaseQuery("conversations", "GET", null, `customer_id=eq.${customerId}&select=*&order=created_at.desc&limit=${limit}`)) || [];
}
...
async function updateConversationContext(customerId: number, data: any) {
  if (!customerId) return;
  const existing = await supabaseQuery("conversation_context", "GET", null, `customer_id=eq.${customerId}&select=*`);
...
function extractOrderDetails(message: string, products: any[]) {
  let productName = null, quantity = 1, customerName = null, phoneNumber = null, deliveryAddress = null;
...
async function generateAIResponse(psid: string, messageText: string): Promise<string> {
  try {
    ...
    const mentionedProduct = products.find((p: any) => messageText.toLowerCase().includes(p.name.toLowerCase()));
...
${productContext}`;
    ...
    if (messageText.includes("မှာမယ်") || messageText.includes("ယူမယ်") || aiReply.includes("အော်ဒါ")) {
      ...
      const product = products.find((p: any) => p.name === details.productName);
      if (product && details.customerName && details.phoneNumber && details.deliveryAddress) {
        if (product.stock_quantity <= 0) {
          return `စိတ်မရှိပါနဲ့ခင်ဗျာ၊ ${product.name} က လက်ရှိ Stock ပြတ်နေလို့ Pre-order အနေနဲ့ပဲ မှတ်သားထားပေးပါမယ်။ ၇-၁၀ ရက်အတွင်း ပို့ပေးပါမယ်ခင်ဗျာ။`;
        }
      ...
      try {
        ...
        for (const entry of body.entry || []) {
          for (const event of entry.messaging || []) {
            ...
            tasks.push((async () => {
              // ✅ Duplicate check — same message_id ဆိုရင် skip
              if (messageId) {
              ...
              if (event.message && event.message.text) {
                ...
                await notifyOwnerDashboard(
                  customer.id, "non_text_message", "Non-text Message",
                  `Customer sent a ${Object.keys(event.message)[0]}`
                );
              }
            })());
          }
